...
def bs_euler_implicit(option, num_paths, num_steps):
    """
    Calculate the price of European options using the Euler implicit method.
    ...
    for i in range(1, num_steps + 1):
        paths[:, i] = (paths[:, i - 1] + sigma * paths[:, i - 1] * np.sqrt(dt) * Z[:, i - 1]) / (1 - r * dt)
...
def calculate_euler_implicit_price(S, K, T, r, sigma, num_paths, num_steps, option_type):
    option = Option(S, K, T, r, sigma, option_type)
    true_price = option.black_scholes()
    mc_price, _ = bs_euler_implicit(option, num_paths, num_steps)
    return mc_price, abs(mc_price - true_price)
...
def bs_convergence_speed(option, num_paths, num_steps):
    """
    Calculate the prices of European options using the Black-Scholes, Monte Carlo, Euler Maruyama, Implicit Euler
    and Milstein Scheme methods and their respective errors compared to the Black-Scholes price.
    ...
    tuple
        ...
        - results: Contains the prices of the option calculated using the Black-Scholes, Monte Carlo, Euler Maruyama, Implicit Euler
        and Milstein Scheme methods.
    ...
    implicit_euler_args = [(S, K, T, r, sigma, num_paths, step, option_type) for step in steps]
    ...
    with Pool() as pool:
        ...
        pool.starmap(calculate_euler_implicit_price, dummy_args)  # Warm-up Implicit Euler
        ...
        start_time = time.time()
        euler_implicit_results = pool.starmap(calculate_euler_implicit_price, implicit_euler_args)
